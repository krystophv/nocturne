...
public class DeviceStatusDecomposer : DecomposerBase, IDeviceStatusDecomposer, IDecomposer<DeviceStatus>
{
    ...
    // Stores written before one span per period can hold hundreds of open spans at one instant, so the containing query pages by this.
    ...
    private static StateSpan BuildOverrideSpan(DeviceStatus ds, string? legacyId)
    {
        ...
        return new StateSpan
        {
            ...
            Source = ds.Device,
            OriginalId = legacyId,
            // Empty rather than null for a bare {active: true}, so the next snapshot matches it.
            Metadata = BuildOverrideMetadata(ds.Override) ?? new Dictionary<string, object>(),
        };
    }
        ...
        DeviceStatus ds, string? legacyId, V4Models.DecompositionResult result, CancellationToken ct)
    {
        ...
        var observed = ds.Override!.Active == true
            ? BuildOverrideMetadata(ds.Override) ?? new Dictionary<string, object>()
            : null;
        ...
        bool Unchanged(StateSpan span) =>
            ...
            && (span.Source != ds.Device
                || span.Metadata!.OverrideScaleFactor() == observed.OverrideScaleFactor());
